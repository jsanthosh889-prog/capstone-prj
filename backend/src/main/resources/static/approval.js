...
const API_PO_URL = `${getApiBaseHost()}/api/purchase-orders`;
    ...
    pendingList.innerHTML = pendingPOs.map(po => `
        <div class="po-item-card" id="po-card-${po.id}" onclick="selectPO(${po.id})">
            <div style="display: flex; justify-content: space-between; margin-bottom: 0.25rem;">
                ...
                <span style="font-weight: 600;">₹${Number(po.amount).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
            </div>
...
window.selectPO = function(id) {
    ...
    detailAmount.innerText = `₹${Number(selectedPO.amount).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
