...
async function fetchPagedPOs() {
    try {
        ...
        const url = new URL(API_PO_URL, window.location.origin);
    ...
    poListBody.innerHTML = pos.map(po => {
        ...
        return `
            <tr>
                ...
                <td style="font-weight: 500;">₹${Number(po.amount).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
